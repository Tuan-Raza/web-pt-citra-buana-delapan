...
$query_sql = "SELECT bm.*, db.nama_barang, db.kode_sku 
              FROM barang_masuk bm 
              JOIN data_barang db ON bm.id_barang = db.id_barang 
              WHERE 1=1";

if ($search != '') {
    $query_sql .= " AND (bm.id_transaksi LIKE '%$search%' OR bm.supplier LIKE '%$search%' OR db.nama_barang LIKE '%$search%')";
}
$query_sql .= " ORDER BY bm.id_masuk DESC";
...
<head>
    ...
    <title>Barang Masuk - PT Citra Buana Delapan</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .btn-green { background-color: #10b981; color: white; border: none; padding: 12px 24px; border-radius: 8px; font-size: 14px; font-weight: 600; cursor: pointer; text-decoration: none; transition: background 0.2s; box-shadow: 0 4px 6px rgba(16, 185, 129, 0.2); }
        ...
        /* MODAL SUCCESS CUSTOM MENGGANTIKAN ALERT BROWSER */
        ...
        .toast-icon { width: 80px; height: 80px; background: #dcfce7; color: #10b981; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 40px; margin: 0 auto 20px auto; font-weight: bold; box-shadow: 0 0 20px rgba(16, 185, 129, 0.2);}
        ...
        .btn-toast { background: #10b981; color: white; border: none; padding: 14px 30px; border-radius: 12px; font-size: 15px; font-weight: 600; cursor: pointer; display: inline-block; transition: all 0.2s; width: 100%; box-shadow: 0 4px 15px rgba(16, 185, 129, 0.3);}
        .btn-toast:hover { background: #059669; transform: translateY(-2px);}
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="menu">
            ...
            <li><a href="barang_masuk.php" class="active">Barang Masuk</a></li>
        ...
        <div class="table-section">
            <div class="filter-bar">
                <form method="GET" action="barang_masuk.php">
                    <input type="text" name="cari" class="search-input" placeholder="Cari ID Transaksi atau Supplier..." value="<?= $search ?>" onchange="this.form.submit()">
                </form>
                <a href="tambah_barang_masuk.php" class="btn-green">+ Catat Penerimaan</a>
            </div>
                ...
                <tbody>
                    ...
                    if(mysqli_num_rows($query) > 0) {
                        while($row = mysqli_fetch_assoc($query)){
                            $tgl = date("d M Y", strtotime($row['tanggal_masuk']));
                            ...
                            echo "<td><strong style='color:#10b981'>+ {$row['jumlah']}</strong></td>";
                            echo "<td><span class='badge-success'>Selesai</span></td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='6' style='text-align:center; padding:40px; color:#64748b;'>Tidak ada data transaksi masuk.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
    ...
    <div class="toast-overlay" id="successToast">
        <div class="toast-box">
            ...
            <h3>Penerimaan Berhasil!</h3>
            <p>Data barang masuk telah berhasil dicatat dan stok gudang otomatis bertambah.</p>
    ...
    <script>
        function tutupToast() {
            ...
            // Bersihkan URL dari ?status=sukses tanpa merefresh halaman
